...
<template>
    <div class="flex mb-4 place-content-end">
        <div class="px-4 py-2 text-white rounded-l-md rounded-r-md border bg-indigo-600 cursor-pointer hover:bg-indigo-700">
            <router-link :to="{ name: 'jabatan.create' }" class="text-sm font-medium">Tambah Jabatan</router-link>
        </div>
    </div>
        ...
        <table class="min-w-full border divide-y divide-gray-200">
            ...
            <tr>
                <th class="px-6 py-3 bg-gray-50">
                    <span
                        class="text-xs font-medium leading-4 tracking-wider text-left text-gray-500 uppercase">Nama Jabatan</span>
                </th>
                <th class="px-6 py-3 bg-gray-50">
                    <span
            ...
            <template v-for="item in jabatans" :key="item.id">
                <tr class="bg-white">
                    ...
                    <td class="inline-flex text-right rounded-md shadow-sm mr-2" role="group" >
                            <button class="mt-1 py-2 px-4 text-sm font-medium text-gray-900 bg-cyan-500 rounded-l-md border border-gray-200 hover:bg-gray-100 hover:text-blue-700 focus:z-10 focus:ring-2 focus:ring-blue-700 focus:text-blue-700 dark:bg-gray-700 dark:border-gray-600 dark:text-white dark:hover:text-white dark:hover:bg-gray-600 dark:focus:ring-blue-500 dark:focus:text-white">
                                <router-link :to="{ name: 'jabatan.edit', params: { id: item.id } }">
                                Edit</router-link>
                            </button>
                            <button class="mt-1 py-2 px-4 text-sm font-medium text-white bg-rose-400 rounded-r-md border border-gray-200 hover:bg-gray-100 hover:text-blue-700 focus:z-10 focus:ring-2 focus:ring-blue-700 focus:text-blue-700 dark:bg-gray-700 dark:border-gray-600 dark:text-white dark:hover:text-white dark:hover:bg-gray-600 dark:focus:ring-blue-500 dark:focus:text-white"
...
const { jabatans, getJabatans, destroyJabatan } = useJabatan()
...
const deleteJabatan = async (id) => {
    ...
    await destroyJabatan(id)
    await getJabatans()
}
onMounted(getJabatans)
</script>